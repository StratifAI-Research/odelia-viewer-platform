...
# ODV-214 vendoring patch: ODELIA_Dataset3D is imported lazily inside the
# training helpers below. The serving block only needs create_model /
# load_environment_variables, and does not ship the training-side data/
# package (torchio/pandas/blosc2). Keeping this import at module top would
# drag those deps in just to build a model.
...
def build_odelia_manifests(env_vars=None):
    from data.datasets import ODELIA_Dataset3D  # ODV-214: lazy (training-only)
