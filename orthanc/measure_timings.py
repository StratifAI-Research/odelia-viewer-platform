...
class TimingProfiler:
    ...
    def _print_summary(self, total_duration: float):
        ...
        logger.info("=" * 80)
        logger.info(
            f"Total End-to-End Duration: {total_duration:.2f}ms ({total_duration/1000:.2f}s)"
        )
        logger.info("")
...
def get_study_info(orthanc_url: str, study_id: str) -> dict:
    ...
    if "." in study_id:
        logger.info("Study ID looks like DICOM UID, looking up Orthanc ID...")
        try:
            lookup_response = requests.post(
                f"{orthanc_url}/tools/lookup", data=study_id, headers={"Content-Type": "text/plain"}
            )
...
def delete_study_from_router(orthanc_router_url: str, study_instance_uid: str) -> bool:
    """
    ...
    try:
        ...
        if not study_results:
            logger.info("Study not found in router (clean slate)")
            return True
...
def send_to_ai_and_profile(
    ...
    # Prepare request
    payload = {"study_id": study_id, "target": target, "target_url": target_url}
    ...
    try:
        ...
        profiler.record(
            ...
            metadata={"study_id": study_id, "target": target, "status_code": response.status_code},
        )
...
Examples:
  ...
  python measure_timings.py --study-id abc123 --target orthanc-router
    ...
    parser.add_argument(
        "--study-id", required=True, help="Orthanc study ID or DICOM StudyInstanceUID to process"
    )
    ...
    parser.add_argument("--target-url", help="Target DICOM URL (default: auto-detect from target)")

    parser.add_argument("--series-uids", nargs="+", help="Optional: Specific series UIDs to send")
    ...
    try:
        ...
        logger.info(
            f"Study UID (DICOM): {study_info.get('MainDicomTags', {}).get('StudyInstanceUID', 'N/A')}"
        )
        logger.info(
            f"Patient: {study_info.get('PatientMainDicomTags', {}).get('PatientName', 'N/A')}"
        )
        ...
        if results_received:
            ...
            for s in final_series:
                logger.info(
                    f"  • {s['description']} ({s['modality']}) - {s['instances']} instances"
                )
