...
def create_model(logger=None, model_name: str = None, num_classes: int = 3, 
    ...
    # ODV-214 vendoring patch: upstream MediSwarm hard-raised without a GPU
    # (a training-time assumption). For the serving block, MODEL_DEVICE must be
    # set explicitly to 'cpu' or 'cuda' ('cuda' requires an available GPU);
    # anything else, including unset, fails loudly so a device is never chosen
    # implicitly. Kept in lockstep with config.resolve_device().
    _model_device = os.environ.get("MODEL_DEVICE", "").strip().lower()
    if _model_device == "cpu":
        device = "cpu"
    elif _model_device == "cuda":
        if not torch.cuda.is_available():
            raise RuntimeError("MODEL_DEVICE=cuda but no CUDA GPU is available.")
        device = "cuda"
    else:
        raise RuntimeError(
            f"MODEL_DEVICE must be set to 'cpu' or 'cuda' (got {os.environ.get('MODEL_DEVICE')!r})."
        )

    logger.info(f"Running code version {env_vars['mediswarm_version']}")
    logger.info(f"Model name: {model_name} (device: {device})")
